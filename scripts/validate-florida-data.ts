...
async function main() {
  ...
  try {
    ...
    const coordPercentage = facilitiesWithCoords.length > 0 ? 
      ((facilitiesWithCoords.length / floridaRecords.length) * 100).toFixed(1) : '0.0';
    console.log(`\nCoordinate data: ${facilitiesWithCoords.length}/${floridaRecords.length} facilities (${coordPercentage}%)`);
